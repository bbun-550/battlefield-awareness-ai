...
def get_action():
    ...
    global TOTAL_SHOT_COUNT, LAST_FIRE_TIME
    ...
    if FIRE_MODE:
        ...
        if aim_good_enough(ctrl["ex"], ctrl["ey"]):
            ...
            if time_since_aim >= FIRST_FIRE_DELAY and time_since_fire >= RELOAD_COOLDOWN:
                ...
                TOTAL_SHOT_COUNT += 1  
                LAST_FIRE_TIME = time.time() # GIF 쿨타임 시작
                print(f"🚀 Firing Command Sent! (Total GUI Count: {TOTAL_SHOT_COUNT})")
        else:
            FIRE_AIM_START = None
...
def update_bullet():
    ...
    FIRE_COUNT += 1
    print(f"💥 Impact Confirmed! Game Progress: {FIRE_COUNT}/3")
