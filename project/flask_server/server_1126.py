...
def load_map():
    ...
    for ob in data.get("obstacles", []):
        ...
        pos = ob.get("position", {})
        obj_data = {"name": name, "x": float(pos.get("x",0)), "y": float(pos.get("y",0)), "z": float(pos.get("z",0))}
        
        # 이동 장애물
        if any(k in name for k in ["tank", "car", "rock"]):
...
def info_get(): return jsonify({"pos":{"x":server_player_pos[0], "y":server_player_pos[1], "z":server_player_pos[2]}})
